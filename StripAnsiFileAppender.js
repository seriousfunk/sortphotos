/**
 * @class StripAnsiFileAppender
 *
 */
const Logger = require('simple-node-logger').Logger
const AbstractAppender = require('simple-node-logger').AbstractAppender
const dash = require( 'lodash' );
const path = require( 'path' );

const StripAnsiFileAppender = function(options) {
    'use strict';

    const appender = this;
    const fs = options.fs || require( 'fs' );        
    const newline = /^win/.test(process.platform) ? '\r\n' : '\n';
    const typeName = options.typeName || 'StripAnsiFileAppender';
...
module.exports = StripAnsiFileAppender;
